...
public class MerakiInstallCheckTests
{
    ...
    [Fact]
    public void Teams_passes_when_the_major_application_is_excluded()
    {
        const string json = """
            { "custom":[], "majorApplications":[ {"id":"meraki:vpnExclusion/2","name":"Microsoft Teams"} ] }
            """;

        Assert.Equal(CheckVerdict.Pass, MerakiInstallCheck.TeamsBreakout([json]).Verdict);
    }

    [Fact]
    public void Teams_passes_when_a_custom_rule_covers_the_udp_ports()
    {
        const string json = """
            { "custom":[ {"protocol":"udp","destination":"13.107.64.0/18","port":"3478-3481"} ],
              "majorApplications":[] }
            """;
    ...
    [Fact]
    public void Teams_fails_when_nothing_is_excluded()
    {
        const string json = """
            { "custom":[ {"protocol":"tcp","destination":"any","port":"443"} ], "majorApplications":[] }
            """;

        MerakiCheckRow row = MerakiInstallCheck.TeamsBreakout([json]);

        Assert.Equal(CheckVerdict.Fail, row.Verdict);
        Assert.Contains("VPN exclusion", row.Detail, StringComparison.Ordinal);
    }

    [Fact]
    public void Teams_is_skipped_when_the_setting_could_not_be_read()
        => Assert.Equal(CheckVerdict.Skipped, MerakiInstallCheck.TeamsBreakout([]).Verdict);

    [Theory]
    [InlineData("3478", true)]
    [InlineData("3478-3481", true)]
    [InlineData("any", true)]
    [InlineData("443,3479", true)]
    [InlineData("443", false)]
    [InlineData("", false)]
    public void Teams_ports_are_recognised_in_every_notation(string port, bool covered)
        => Assert.Equal(covered, MerakiInstallCheck.CoversTeamsPort(port));

    // ===== イベントログ =====

    private const string EventsJson = """
        { "pageStartAt":"2026-08-17T00:00:00Z", "pageEndAt":"2026-08-17T01:00:00Z",
          "events":[
            {"occurredAt":"2026-08-17T00:10:00Z","type":"dhcp_lease","description":"DHCP lease"},
            {"occurredAt":"2026-08-17T00:20:00Z","type":"vpn_connectivity_change","description":"VPN down"},
            {"occurredAt":"2026-08-17T00:30:00Z","type":"uplink_status_change","description":"WAN1 down"},
            {"occurredAt":"2026-08-17T00:40:00Z","type":"vpn_connectivity_change","description":"VPN up"} ] }
        """;

    [Fact]
    public void Events_warn_and_count_the_ones_worth_looking_at()
    {
        MerakiCheckRow row = MerakiInstallCheck.Events([EventsJson]);

        Assert.Equal(CheckVerdict.Warn, row.Verdict);
        Assert.Contains("vpn_connectivity_change 2 件", row.Detail, StringComparison.Ordinal);

        // 正常な払い出しは数えない（数えると毎回「注意」になる）
        Assert.DoesNotContain("dhcp_lease", row.Detail, StringComparison.Ordinal);
    }

    [Fact]
    public void Events_pass_when_only_ordinary_ones_are_there()
    {
        const string json = """
            { "events":[ {"occurredAt":"2026-08-17T00:10:00Z","type":"dhcp_lease"},
                         {"occurredAt":"2026-08-17T00:11:00Z","type":"settings_changed"} ] }
            """;

        Assert.Equal(CheckVerdict.Pass, MerakiInstallCheck.Events([json]).Verdict);
    }

    [Fact]
    public void Events_are_skipped_when_none_came_back()
        => Assert.Equal(CheckVerdict.Skipped, MerakiInstallCheck.Events(["""{"events":[]}"""]).Verdict);
