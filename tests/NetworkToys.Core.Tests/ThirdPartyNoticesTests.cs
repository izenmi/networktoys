...
public class ThirdPartyNoticesTests
{
    private static string Root()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "NetworkToys.slnx")))
            dir = dir.Parent;

        Assert.True(dir is not null, "リポジトリのルート（NetworkToys.slnx）が見つからない");
    ...
    public void Every_package_is_listed()
    {
        string csproj = File.ReadAllText(
            Path.Combine(Root(), "src", "NetworkToys.App", "NetworkToys.App.csproj"));
