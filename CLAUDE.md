...
- **入れ子は 2 段まで。** 3 段にすると道具を探せなくなる。まとめたタブの中身は
  **見出しを押すと降りてくる縦並びのメニュー**で選ばせる（`OnMainTabsMouseDown`）。
  切り替えの帯は横にも縦にも出さない（`TabPanel` を `Collapsed`）— 帯とメニューの
  二重の入り口になって迷わせるため。自己診断が帯の非表示を確かめている。
