...
/// 上限を決めて古い順に捨てるのが前提。
///
/// <b>スレッド安全ではない。呼び出しは単一スレッド（UI のポンプ）に限ること。</b>
/// ロックも volatile も無いので、別スレッドから読むと書き込み途中の状態が見え、
/// 最新の要素が最古として返ることがある。レポート生成などを ThreadPool へ
/// 逃がすときは、先に UI スレッド側でスナップショットを取ってから渡すこと。
/// </summary>
