...
public static class HandoverStore
{
    public static void Save(string path, HandoverDocument document)
    {
        ...
        File.WriteAllText(path, JsonSerializer.Serialize(document, PingWatcherJsonContext.Default.HandoverDocument));
    }
    ...
    public static HandoverDocument? LoadAndDelete(string path)
    {
        try
        {
            ...
            HandoverDocument? document =
                JsonSerializer.Deserialize(json, PingWatcherJsonContext.Default.HandoverDocument);
