...
internal static class TabHelp
{
    ...
    /// <summary>Meraki ─ 利用率</summary>
    public const string MerakiUsage =
        """
        Meraki ─ 利用率

        この画面ですること
        ・MX の利用率（device utilization）を、拠点をまたいで一覧にする
        ・選んだ拠点の回線（WAN）の使われ方を、期間の中の推移で見る

        利用率の読み方
        ・33% まで ● 余裕 ／ 66% まで ⊘ やや高い ／ それ以上 ✕ 高い
        ・「—」は取れなかったもの。0% とは意味が違う
        ・答えるのはプライマリの MX だけ。それ以外は理由を備考に出す

        知っておくとよいこと
        ・MX の一覧は「機器」タブで取ったものを使う。先にあちらを取得しておく
        ・回線の推移は縦軸 0 から。上限はその期間の最大に合わせる
        ・グラフだけでは数字が読めないので、最大と平均を上に添えている
    ...
    public const string MerakiSites =
        ...
        ・その拠点で実際に使われているセグメント（スタティックルート）を並べる
        ...
        ・拠点の数だけ問い合わせるので時間がかかる。多いときは途中で打ち切る
    ...
    public const string MerakiInstallCheck =
        ...
        こんなときに
        ・MX を入れた当日、現地で「ちゃんと上がっているか」を一度に見たいとき
        ...
        ・気になるイベントが出ていないか／端末が繋がっているか

        気をつけること
        ・MX のポート速度は API から取れない。ダッシュボードを見て ○ か ✕ を押す
        ・Teams は設定の確認まで。実際に UDP が抜けるかは試験タブで見る
    ...
    public const string AciConfig =
        ...
        　設定バックアップを、種別を選んで一覧にする

        こんなときに
        ・台帳を作りたいとき（ファブリック構成に型番・シリアル・版が入る）
        ・「なぜこのポートでこの VLAN が通らないか」を辿るとき（AEP や VLAN プール）
        ・バックアップが取れているかを確かめたいとき

        知っておくとよいこと
        ・列は 4 種別で共通（種別／名前／親／状態／備考）。種別ごとに表を変えない
        ・VLAN プールの範囲は「備考」に出る
    ...
    public const string Wlc =
        ...
        ・無効な WLC では「SSH で取得」が保険になる。ただし表にはせず、
        　show の出力をそのまま見せる（版によって桁がずれるため）
    ...
    public const string MerakiClients =
        ...
        知っておくとよいこと
        ・「全拠点」を入れると、拠点をまたいで数える（拠点の数だけ問い合わせるので時間がかかる）
