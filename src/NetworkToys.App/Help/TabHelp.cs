...
internal static class TabHelp
{
    ...
    public const string AciPorts =
        ...
        ・その口にどの EPG と VLAN が載っているか、タグ付けはどうかを並べる
        ...
        ・1 本の口に複数の EPG が載っていれば、まとめて並べる（全文はマウスを載せる）

        ポートチャネル
        ・束(po1 など)そのものも 1 行として出す。EPG は束の側に付くため
        ・メンバーの口には所属する束を出し、束に載った EPG もその口のものとして出す
    ...
    public const string AciConfig =
        ...
        ・「なぜこの口でこの VLAN が通らないか」を辿るとき（AEP や VLAN プール）
        ...
        作業前後で見比べる
        ・テナントを選んで「設定を書き出す」と、その枝を丸ごと取って並べ直す
        ・「作業前へ」「作業後へ」で差分比較タブに入る（対象は「そのまま比較」）
        ・設定だけを取る（稼働値を混ぜない）。混ぜると変えていなくても差分になる
        ・並び順は APIC 任せなので、こちらで並べ替えてから比べる
        ・テナントの一覧は上の「取得」を押すと埋まる
