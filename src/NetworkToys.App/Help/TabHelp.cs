...
internal static class TabHelp
{
    ...
    public const string MerakiUsage =
        ...
        この画面ですること
        ・機器の使われ具合を、多い順に棒グラフで並べる

        2 つの見方
        ・利用率（MX）— MX の平均利用率。回線や機器が張り付いていないかを見る
        ・通信量（全機器）— 期間内に流れた量。よく使われている機器を見つける

        知っておくとよいこと
        ・棒の長さは、一番大きいものを 1 とした比率。単位が違っても同じ見た目で読める
        ・組織や契約によっては、この要約を持っていないことがある（その旨が状態欄に出る）
        ・期間はクライアント一覧と同じ選択肢を使う
