...
internal static class TabHelp
{
    ...
    public const string MerakiUsage =
        ...
        この画面ですること
        ・選んだ拠点の使われ具合を、期間の中でどう動いたかの折れ線で見る

        2 つの見方
        ・AP のチャンネル使用率 — 電波の混み具合。機器と帯域をまたいで平均した値
        ・回線の使用量（WAN）— その拠点の WAN に流れた量。bps に直して出す

        知っておくとよいこと
        ・縦軸は 0 から。利用率は 100% で固定、帯域はその期間の最大に合わせる
        ・グラフだけでは数字が読めないので、最大と平均を上に添えている
        ・拠点や契約によっては、この推移を持っていないことがある（状態欄に出る）
