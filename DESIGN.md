...
| タブ | 内容 |
|---|---|
| 宛先 | 宛先リストの一括編集(EXPing 書式) |
| Ping | 並列 ping。状態バッジ / RTT / ロス率 / スパークライン。既定画面 |
| 無線 | 接続中AP詳細 / RSSI時系列 / 周辺APのチャンネル混雑グラフ |
| 差分 | 機器の show 出力を作業前後で突き合わせる |
| 遮断 | WFP が落とした通信の一覧(管理者権限) |
| IP設定 | IPv4 設定の切替 + プロキシ設定 |
| **調べる** | TCP / 経路 / スキャン / DNS / SNMP / 接続 / 計算 |
| **機器** | 収集 / パース |
| **受ける** | FTP / TFTP / SFTP / syslog / Trap |
| **Meraki** | ネットワーク / 機器 / アップリンク / クライアント |
| 試験 | 業務確認試験(プロキシを切り替えて回す) |
| 記録 | Ping と TCP の結果を HTML / テキスト / CSV へ |
