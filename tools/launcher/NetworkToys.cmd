@echo off
rem ============================================================
rem  NetworkToys（軽量版）の起動用。
rem
rem  軽量版は .NET デスクトップランタイムが別途要る。入っていないと
rem  NetworkToys.exe は Windows 側の起動処理で止まり、英語のダイアログが
...
set "EXE=%~dp0NetworkToys.exe"

if not exist "%EXE%" (
    powershell -NoProfile -Command "Add-Type -AssemblyName PresentationFramework; [void][System.Windows.MessageBox]::Show('NetworkToys.exe が見つかりません。このファイルは NetworkToys.exe と同じフォルダに置いてください。', 'NetworkToys', 'OK', 'Warning')"
    exit /b 1
)
...
powershell -NoProfile -Command ^
  ...
  "     'NetworkToys の軽量版はこれが別途必要です。入手ページを開きますか？' + $nl + $nl +" ^
  "     '・「.NET Desktop Runtime」の x64 版を入れてください' + $nl +" ^
  "     '・入れずに使いたい場合は、ランタイム同梱の単一exe版をお使いください';" ^
  "if ([System.Windows.MessageBox]::Show($m, 'NetworkToys', 'YesNo', 'Information') -eq 'Yes') {" ^
  "  Start-Process '%URL%' }"
